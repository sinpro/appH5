...
import { rpc, getDeviceAndAppInfo, showSharePad, alert } from '@core/bridge'
...
const mixins = {
	...
	async created(){
		...
		const { macAdress } = await getDeviceAndAppInfo()
		rpc('JA00008', {
		...
		Promise.all([this.getActivityDetail(), this.getActivityTimes()]).then(res => {
			...
			status ? this.ready = true : alert('活动结束', desc)
		})
	},
	methods: {
		//客户端内分享
		popShare(cb){
			...
			const {shareUrL, shareTitle, shareDesc, shareCover} = this.activityDetail
			const customerId = this.getParamInfo('customerId')
			const marketId = this.getParamInfo('marketId')
			const mobileNo = this.getParamInfo('mobileNo')
			let paramstr = `customerId=${customerId}&marketId=${marketId}&mobileNo=${mobileNo}`
			let shareUrLStr = (shareUrL || '').indexOf('?') > -1 ? (shareUrL + '&' + paramstr) : (shareUrL + '?' + paramstr)
